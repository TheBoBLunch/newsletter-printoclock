...
class NewslettersController extends AbstractController
{
    ...
    public function index(Request $request, MailerInterface $mailer): Response
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            //Systèmatiquement je crée un token 'unique' liée à l'inscription d'un user
            //Ca nous permet de voir si l'utilisateur (user) est passé par l'interface pour s'incrire
    ...
    /**
     * Envoyer à chaque nouvel utilisateur
     * un mail de confirmation d'inscription à la newsletter
    ...
    /**
     * Prepare une nouvelle newsletter
    ...
    public function unsubscribe(Users $user, Newsletters $newsletter, $token): Response
    {
        ...
        if(count($user->getCategories()) > 1){
            ...
            //Si le user se décinscrit de la seule catégorie de newsletter à laquelle il est inscrit
            //c'est qu'il n'est plus interressé par nos services,
            //donc on le désinscrit
            $em->remove($user);
        }
        $em->flush();
